...
function Quizz({ questionArray }) {
  const [currentQuestionIndex, setCurrentQuestionIndex] = useState(0);
  const [answers, setAnswers] = useState(
    Array(questionArray.length).fill(null)
  );
  ...
  const handleSubmit = () => {
    ...
    answers.forEach((answer, index) => {
      if (answer === questionArray[index].correct) {
  ...
  useEffect(() => {
    const handleKeyDown = (event) => {
      const options = questionArray[currentQuestionIndex].options;
    ...
    <>
      ...
      <section className="mt-36">
        ...
        <div className="flex relative justify-between px-10 py-2 left-20 gap-2 text-3xl bg-slate-200 w-[86rem] rounded-t-lg items-center">
          ...
          <div>
            <p className="text-3xl">{currentQuestion.category}</p>
          </div>
          <div>
        ...
        {score !== null && (
          <div className="mt-10 text-center text-2xl font-bold">
            Your Score: {score} / {questionArray.length * 4}
          </div>
        )}
      </section>
    </>
